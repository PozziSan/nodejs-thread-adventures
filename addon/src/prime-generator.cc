...
std::vector<std::string> generatePrimesThreads(int threadCount, int count, const std::string startingNumber, bool format, bool log) {
    ...
    for (int i = 0; i < threadCount; i++) {
        ...
        mpz_set_str(gmpValue, startingNumber.c_str(), 10);
        ...
        threads.push_back(std::thread(calc, count / threadCount, updatedStartValue, format, log));
        mpz_clear(gmpValue);
    }
